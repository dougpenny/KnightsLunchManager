...
class TransactionDepositForm(forms.Form):
    ...
    transactee = TransacteeSelectField(
        widget=forms.Select(
            attrs={"class": "transactee-select-ajax", "style": "width: 100%"}
        )
    )
...
class TransactionOrderForm(forms.Form):
    transactee = forms.CharField(widget=forms.HiddenInput())
    menu_item = forms.CharField(widget=forms.HiddenInput())
    submitted = forms.DateTimeField(widget=forms.HiddenInput())
