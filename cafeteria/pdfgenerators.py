...
def entree_report_by_period(lunch_periods: Dict) -> FileResponse:
    ...
    margin = 0.5 * inch
    document = platypus.BaseDocTemplate(
        buffer,
        pagesize=letter,
        rightMargin=margin,
        leftMargin=margin,
    ...
    frame = platypus.Frame(
        ...
        document.width,
        document.height - title_frame_height - 1.0 * inch,
        id="entree-frame",
    )
    frames.append(frame)
    ...
    for period in lunch_periods:
        if len(lunch_periods[period]) > 0:
            ...
            if not period.floating_staff:
                for item in lunch_periods[period]:
                    if item.pizza:
                        ...
                        if left_over_slices == 0:
                            data.append(
                                platypus.Paragraph(
                                    "<u>{}</u><br/><br/><b>{}</b> {}".format(
                                        item, pizzas, pizza_string
                                    ),
                                    entree_style,
                                )
                            )
                            ...
                            data.append(
                                platypus.Paragraph(
                                    "<u>{}</u><br/><br/><b>{}</b> {}<br/><br/><b>{}</b> {}".format(
                                        item,
                                        pizzas,
                                        pizza_string,
                                        left_over_slices,
                        ...
                        data.append(
                            platypus.Paragraph(
                                "{} - <b>{}</b>".format(
                                    item, lunch_periods[period][item]
                                ),
                                entree_style,
                            )
                        )
                ...
                item_orders = collections.OrderedDict(
                    sorted(
                        item_orders.items(), key=lambda menu_item: menu_item[0].sequence
                    )
                )
                for item in item_orders:
                    content = [
                        platypus.Paragraph(
                            "<b><u>{}</u></b>".format(item.name), staff_style
                        )
                    ]
    ...
    buffer.seek(0)
    today = timezone.now()
    report_name = "lunch_periods_{}-{}-{}.pdf".format(
...
def order_report_for_limited_items(limited_item_orders: Dict) -> FileResponse:
    ...
    for menu_item, orders in limited_item_orders.items():
        ...
        for order in orders:
            ...
            customer = (
                customer
                + f' - {order.transactee.grade if order.transactee.grade else "Staff"}'
            )
            data.append(platypus.Paragraph(customer, customer_style))
    document.build(data)
    buffer.seek(0)
    today = timezone.now()
    report_name = "limited_items_{}-{}-{}.pdf".format(
        today.year, today.month, today.day
    )
    return FileResponse(buffer, as_attachment=True, filename=report_name)
...
def lunch_card_for_users(profiles: List[Profile]) -> FileResponse:
    ...
    frames = [
        platypus.Frame(
            document.leftMargin,
            card_height - title_height - top_offset,
            document.width,
            title_height,
            id="title-frame",
        )
    ...
    frames.append(
        platypus.Frame(
            document.leftMargin,
            card_height - title_height - (12 * mm),
            document.width,
            role_height,
            id="role-frame",
        )
    ...
    frames.append(
        platypus.Frame(document.leftMargin, 0, 12 * mm, 9 * mm, id="number-frame")
    )
    frames.append(
        platypus.Frame(
            card_width - qr_size,
            document.bottomMargin - margin,
            qr_size,
            qr_size,
            id="qr-frame",
        )
    ...
    image_path = (
        Path(__file__).resolve(strict=True).parent / "report_images/knights-head.jpg"
    )
    ...
    for profile in profiles:
        ...
        if profile.cards_printed > 1:
            ...
            data.append(
                platypus.Paragraph("R{}".format(profile.cards_printed), small_style)
            )
...
def orders_report_by_homeroom(todays_orders: List) -> FileResponse:
    ...
    no_orders_frame = platypus.Frame(
        document.leftMargin,
        document.height / 2.0,
        document.width,
        1.0 * inch,
    ...
    for frame in range(3):
        ...
        column = platypus.Frame(
            left_margin,
            document.bottomMargin + item_frame_height + entree_frame_height,
            frame_width,
            student_frame_height,
            id="student-frame-{}".format(frame),
        )
    ...
    for frame in range(2):
        ...
        column = platypus.Frame(
            left_margin,
            item_frame_height + document.bottomMargin,
            frame_width,
            entree_frame_height,
            id="entree-frame-{}".format(frame),
        )
        frames.append(column)
    ...
    for orders in todays_orders:
        ...
        for item in orders["orders"]:
            ...
            if item.menu_item in item_counts:
                item_counts[item.menu_item] = (
                    item_counts[item.menu_item] + item.quantity
                )
            else:
                item_counts[item.menu_item] = item.quantity
        item_orders = collections.OrderedDict(
            sorted(item_orders.items(), key=lambda menu_item: menu_item[0].sequence)
        )
        item_counts = collections.OrderedDict(
            sorted(item_counts.items(), key=lambda menu_item: menu_item[0].sequence)
        )
        ...
        if len(item_orders) > 0:
            ...
            for item in item_orders:
                content = [
                    platypus.Paragraph(
                        "<b><u>{}</u></b>".format(item.name), normal_style
                    )
                ]
            ...
            for item in item_counts:
                if item.category == MenuItem.ENTREE:
                    data.append(
                        platypus.Paragraph(
                            "{} - <b>{}</b>".format(item.short_name, item_counts[item]),
                            entree_count_style,
                        )
                    )
                else:
                    item_content.append(
                        platypus.Paragraph(
                            "{} - <b>{}</b>".format(item.name, item_counts[item]),
                            item_count_style,
                        )
                    )
