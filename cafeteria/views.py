...
from cafeteria.pdfgenerators import (
    ...
    orders_report_by_homeroom,
    order_report_for_limited_items,
)
                    ...
                    for menu_line_item in transaction.line_item.all():
                        if menu_line_item.menu_item.limited:
                            ...
                            if todays_limited_items:
                                limited_item_count = todays_limited_items.get(
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
                    ...
                    if len(ordered_items_list) < 1:
                        ...
                        for item in counted_items:
                            if not item_limit_reached(item):
                                ...
                                description = (
                                    description + f"({counted_items[item]}) {item.name}"
                                )
                                cost = cost + (item.cost * counted_items[item])
                            else:
                                messages.warning(
                                    request,
                                    f"{item.name} has already sold out. Please select a different item.",
                                )
                                return redirect("home")
                        if todays_transaction(request.user.profile):
                            messages.warning(
                                request, "Your have already placed an order today."
                            )
                            return redirect("todays-order")
                        try:
                            ...
                            for item in counted_items:
                                transaction.menu_items.add(
                                    item,
                                    through_defaults={"quantity": counted_items[item]},
                                )
                                if item.limited:
                                    todays_limited_items = (
                                        cache.get(f"{today}")
            ...
            queryset = MenuItem.objects.filter(
                days_available__name=today.strftime("%A")
            ).filter(app_only=False)
            ...
            context["formset"] = OrderFormSet(
                form_kwargs={"queryset": queryset}, prefix="order_form"
            )
    else:
        context["user"] = None
        ...
        for orders in todays_orders:
            ...
            for group in order_groups:
                ...
                for student in orders["orders"].filter(
                    menu_item__short_name=group["menu_item__short_name"]
                ):
